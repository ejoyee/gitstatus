...
(function () {
  ...
  // ✅ URL 기반 대상 문서 판별: 저장된 spreadsheetId와 현재 탭 URL 매칭
  chrome.storage.sync.get({ sheet: { spreadsheetId: '' } }, (st) => {
    try {
      const targetId = st.sheet?.spreadsheetId || '';
      const url = location.href;

      // 대상 문서가 아니면 종료
      if (!targetId || !url.includes(targetId)) return;

      // 대상 문서라면 버튼 주입
      injectButtons();
    } catch (e) {
      console.warn('[GitCheck][content] guard error', e);
    }
  });
  ...
  function injectButtons() {
    ...
    Object.assign(viewBtn.style, {
      position: 'fixed', right: '24px', bottom: '74px',
      zIndex: 999999,
    ...
    function showModal(summary, previewDetail) {
      ...
      for (const row of rows) {
        ...
        tr.innerHTML = `
          ...
          <td style="border-bottom:1px solid #f1f1f1;padding:6px 4px;max-height:120px;overflow:auto">
